...
# Force repo-local Hermes home for this process (never ~/.hermes by default).
apply_hermes_isolation()

STATIC_DIR = Path(__file__).parent / "static"
REPO_ROOT = repo_root()
_MEDIA_PATHS = ensure_media_layout()
MEDIA_ROOT = Path(_MEDIA_PATHS["root"])
MEDIA_IMAGES = Path(_MEDIA_PATHS["images"])
MEDIA_VIDEOS = Path(_MEDIA_PATHS["videos"])
MEDIA_IDENTITY_ASSETS = Path(_MEDIA_PATHS["identity_assets"])
MEDIA_IDENTITY_TEMPLATES = Path(_MEDIA_PATHS["identity_templates"])
NEXUS_DB = REPO_ROOT / ".cinesmith-nexus" / "cinesmith.db"
SCRIPT_PROJECTS_DIR = REPO_ROOT / "data" / "scripts"
SCRIPT_PROJECTS_DIR.mkdir(parents=True, exist_ok=True)

app = FastAPI()
_NEXUS_HANDLERS: Optional[CinesmithMCPHandlers] = None

# Structured errors + self-contained product routes (domain routers register at end)
try:
    from dashboard.errors import register_exception_handlers
    from dashboard.routes import register_extra_routes

    register_exception_handlers(app)
    register_extra_routes(app)
except Exception as _route_reg_err:  # pragma: no cover - boot diagnostics
    print(f"[CINESMITH] extra route registration deferred: {_route_reg_err}")
...
def _require_config_write_access(request: Request) -> None:
    ...
    expected = str(os.getenv("CINESMITH_CONFIG_WRITE_TOKEN", "") or "").strip()
    provided = str(request.headers.get("x-cinesmith-config-token") or "").strip()
    if expected:
    ...
    if not _is_loopback_host(_client_host(request)):
        raise HTTPException(
            status_code=403,
            detail="config writes are restricted to loopback unless CINESMITH_CONFIG_WRITE_TOKEN is set",
        )
...
async def api_system_readiness():
    """Lightweight readiness for first-run UX and smoke tests.

    Checks isolation, media layout, and optional local services without blocking
    long. Downstream providers (Spark, LM Studio, Kimi) are probed with short
    timeouts so the UI can show green/yellow/red without hanging.
    """
    cfg = get_raw_config()
    ...
    checks: Dict[str, Any] = {
        "isolation": {
            "ok": bool(isolation.get("isolation_ok")),
            "detail": isolation,
        },
        "media": {
            "ok": Path(media_paths["images"]).exists() and Path(media_paths["videos"]).exists(),
            "root": media_paths["root"],
    ...
    # Optional short probes
    async def _probe(url: str, timeout: float = 1.5) -> Dict[str, Any]:
        if not url:
            return {"ok": False, "status": "missing", "url": url}
        try:
            async with httpx.AsyncClient(timeout=timeout) as client:
                resp = await client.get(url)
            return {"ok": resp.status_code < 500, "status": resp.status_code, "url": url}
        except Exception as exc:
            return {"ok": False, "status": "unreachable", "url": url, "error": str(exc)[:160]}

    comfy = str(cfg.get("COMFYUI_PRIMARY", "") or os.getenv("COMFYUI_PRIMARY", "http://127.0.0.1:8188")).rstrip("/")
    lm_host = str(cfg.get("LMSTUDIO_HOST", "") or os.getenv("LMSTUDIO_HOST", "http://127.0.0.1:1234")).rstrip("/")
    if lm_host and not lm_host.startswith("http"):
        lm_host = "http://" + lm_host

    spark_probe, lm_probe = await asyncio.gather(
        _probe(f"{comfy}/system_stats" if comfy else ""),
        _probe(f"{lm_host}/v1/models" if lm_host else ""),
    )
    checks["spark"] = spark_probe
    checks["lmstudio"] = lm_probe

    core_ok = checks["isolation"]["ok"] and checks["media"]["ok"]
    ready_for_images = core_ok and bool(spark_probe.get("ok"))
    ready_for_story = core_ok  # script package can draft offline; frames need spark
    overall = "ready" if ready_for_images else ("partial" if core_ok else "blocked")

    return {
        "status": overall,
        "ready_for_images": ready_for_images,
        "ready_for_story": ready_for_story,
        "ready_for_video": ready_for_images,
        "checks": checks,
        "hints": _readiness_hints(checks, overall),
    }


def _readiness_hints(checks: Dict[str, Any], overall: str) -> List[str]:
    hints: List[str] = []
    isolation = checks.get("isolation", {})
    if not isolation.get("ok"):
        hints.append("Hermes isolation failed — set HERMES_HOME to the repo hermes_home/ or relaunch with scripts/launch_cinesmith.sh")
    if not checks.get("media", {}).get("ok"):
        hints.append("Media root missing — set CINESMITH_MEDIA_ROOT or create media/images and media/videos")
    if not checks.get("spark", {}).get("ok"):
        hints.append("Spark/ComfyUI unreachable — start ComfyUI or fix COMFYUI_PRIMARY in Settings")
    if not checks.get("lmstudio", {}).get("ok"):
        hints.append("LM Studio unreachable — optional for local Director/chat; configure in Settings if needed")
    if not checks.get("config", {}).get("has_kimi_key"):
        hints.append("No Kimi/Director API key detected — cloud planning needs KIMI_API_KEY (or local Director mode)")
    if overall == "ready":
        hints.append("Core stack looks good — open Images or Script Studio and generate.")
    return hints
...
async def api_nexus_query(req: NexusQueryRequest):
    ...
    if not NEXUS_DB.exists():
        raise HTTPException(status_code=503, detail=f"Cinesmith Nexus index missing at {NEXUS_DB}")

    handlers = _get_nexus_handlers()
    res = handlers.handle_cinesmith_query({"query": query})
    if "error" in res:
    ...
    for r in top:
        ...
        if req.include_impact:
            impact = handlers.handle_cinesmith_impact({"asset_id": rid})
...
async def api_nexus_impact(req: NexusImpactRequest):
    ...
    if not NEXUS_DB.exists():
        raise HTTPException(status_code=503, detail=f"Cinesmith Nexus index missing at {NEXUS_DB}")
    handlers = _get_nexus_handlers()
    res = handlers.handle_cinesmith_impact({"asset_id": asset_id})
    if "error" in res:
        raise HTTPException(status_code=500, detail=str(res["error"]))
...
async def api_memory_failure_auto_status():
    """J4 — auto-consolidate status after N failures."""
    from core.hermes.memory.failure_auto_consolidate import get_status

    return get_status()


async def api_memory_failure_auto_run(force: bool = False):
    """J4 — force consolidate failure notes into durable memory."""
    from core.hermes.memory.failure_auto_consolidate import consolidate_failures, get_status

    if force:
        result = consolidate_failures(reason="manual_memory_api")
...
async def studio():
    ...
    if template_index.exists():
        return FileResponse(template_index, headers=headers)
...
def _record_pipeline_event(
    ...
    if source == "fallback" and os.getenv("CINESMITH_LEARN_FROM_FALLBACK", "false").lower() != "true":
        return
    payload = {
    ...
    # J4: count campaign/story failures toward auto-consolidate threshold
    if success is False:
        try:
            from core.hermes.memory.failure_auto_consolidate import maybe_auto_consolidate_from_event

            maybe_auto_consolidate_from_event(payload, root=REPO_ROOT)
        except Exception:
            pass
...
class ScriptProjectSaveRequest(BaseModel):
    ...
    hook_first_dialogue: bool = True
    status: str = "draft"
    # E5 multi-episode series continuity
    series_id: str = ""
    series_title: str = ""
    episode_number: Optional[int] = None
    episode_title: str = ""
    series_continuity: Optional[bool] = None
    continues_from_script_id: str = ""
...
def _script_project_summary(project: Dict[str, Any]) -> Dict[str, Any]:
    try:
        ep_num = int(project.get("episode_number") or 0)
    ...
    return {
        ...
        "series_id": project.get("series_id", "") or "",
        "series_title": project.get("series_title", "") or "",
        "episode_number": ep_num,
        "episode_title": project.get("episode_title", "") or "",
        "series_continuity": bool(project.get("series_continuity")),
        "continues_from_script_id": project.get("continues_from_script_id", "") or "",
    }
...
def _save_script_project_payload(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    from core.script_series import series_fields_from_payload

    series_meta = series_fields_from_payload(payload, existing=existing)
    series_continuity = payload.get("series_continuity")
    if series_continuity is None:
        series_continuity = existing.get("series_continuity")
    if series_continuity is None:
        series_continuity = bool(series_meta.get("series_id"))
    continues_from = str(
        payload.get("continues_from_script_id")
        if payload.get("continues_from_script_id") is not None
        else existing.get("continues_from_script_id")
        or ""
    ).strip()
    meta = {
        **existing,
        ...
        "series_id": series_meta.get("series_id") or "",
        "series_title": series_meta.get("series_title") or "",
        "episode_number": int(series_meta.get("episode_number") or 0),
        "episode_title": series_meta.get("episode_title") or "",
        "series_continuity": bool(series_continuity),
        "continues_from_script_id": continues_from,
    }
    _write_json_atomic(root / "project.json", meta)
...
async def api_recover_comfy_prompt(req: ComfyRecoverPromptRequest):
    ...
    if status.get("status_str") == "error" and not _has_video_output(entry):
...
def _script_package_matches_request(package: Optional[Dict[str, Any]], req: ScriptPipelineStartRequest) -> bool:
    if not isinstance(package, dict) or not package:
        return False
    request_meta = package.get("cinesmith_request") if isinstance(package.get("cinesmith_request"), dict) else {}
...
def _fallback_script_package(req: ScriptDevelopRequest) -> Dict[str, Any]:
    ...
    title = (req.title or "").strip() or "Untitled Cinesmith Film"
...
async def _run_script_pipeline_job(job_id: str, req: ScriptPipelineStartRequest) -> None:
    ...
    try:
        ...
        frame_wait_seconds = int(os.getenv("CINESMITH_STORYBOARD_FRAME_WAIT_SEC", "0") or "0") or int(req.video_wait_seconds or 21600)
        ...
        if req.run_video:
            ...
            if req.wait_for_videos and video_jobs:
                video_wait_seconds = int(os.getenv("CINESMITH_SCRIPT_VIDEO_WAIT_SEC", "0") or "0") or int(req.video_wait_seconds or 21600)
...
async def _request_script_package(req: ScriptDevelopRequest) -> Dict[str, Any]:
    ...
    system_prompt = (
        "You are Hermes Script Architect for CINESMITH NPS. Return only valid JSON. "
    ...
    timeout_sec = max(float(os.getenv("CINESMITH_KIMI_TIMEOUT_SEC", "120")), 180.0)
...
async def _stream_director_shot_generation(req: DirectorGenerateRequest) -> AsyncGenerator[str, None]:
    ...
    try:
        ...
        if os.getenv("CINESMITH_DEV_FALLBACK", "false").lower() != "true":
            yield _script_director_event({
                "type": "error",
                "text": f"Director unavailable; coverage fallback is disabled. Check Settings or set CINESMITH_DEV_FALLBACK=true for demo fallback. {str(e)[:180]}",
            })
            return
...
async def api_script_projects():
    projects = _list_script_projects()
    from core.script_series import group_projects_by_series

    return {
        "status": "ok",
        "projects": projects,
        "series": group_projects_by_series(projects),
    }
...
class ScriptSeriesNewEpisodeRequest(BaseModel):
...
async def api_script_series_list():
    """E5: series groups with ordered episodes (from project meta)."""
    projects = _list_script_projects()
    from core.script_series import group_projects_by_series

    return {"status": "ok", "series": group_projects_by_series(projects), "projects": projects}


async def api_script_series_new_episode(req: ScriptSeriesNewEpisodeRequest):
    """
    E5: create the next episode in a series, carrying tone/runtime and series continuity.
    Does not copy storyboard/video assets — new production, same series identity.
    """
    from core.script_series import draft_next_episode_meta, slug_series_id

    source: Dict[str, Any] = {}
    source_id = (req.source_script_id or "").strip()
    if source_id:
        try:
            source = _load_script_project(source_id)
        except HTTPException:
            source = {}
    if not source:
        # Synthetic source from request series fields
        sid = slug_series_id(req.series_id or req.series_title)
        if not sid:
            raise HTTPException(
                status_code=400,
                detail="source_script_id or series_id/series_title required",
            )
        source = {
            "script_id": "",
            "series_id": sid,
            "series_title": (req.series_title or sid).strip(),
            "title": req.series_title or sid,
            "brief": req.brief or "",
            "tone": "",
            "runtime_seconds": 60,
            "target_scenes": 4,
        }
    all_projects = _list_script_projects()
    draft = draft_next_episode_meta(
        source,
        all_projects=all_projects,
        episode_title=req.episode_title or "",
    )
    if req.title.strip():
        draft["title"] = req.title.strip()
    if req.brief.strip():
        draft["brief"] = req.brief.strip()
    elif not req.copy_brief:
        draft["brief"] = ""
    if req.series_title.strip():
        draft["series_title"] = req.series_title.strip()
    if req.series_id.strip():
        draft["series_id"] = slug_series_id(req.series_id)
    # New script_id each episode
    draft["script_id"] = ""
    project = _save_script_project_payload(draft)
    return {"status": "ok", "project": project, "series_id": project.get("series_id"), "episode_number": project.get("episode_number")}
...
class VideoProcessRequest(BaseModel):
    ...
    # First→Last frame mode: optional explicit end still (pairs with first shot_id).
    # When omitted and workflow is first_last with exactly 2 shot_ids, service
    # treats start=shot_ids[0], end=shot_ids[1].
    end_shot_id: Optional[str] = None


class VideoTextProcessRequest(BaseModel):
    prompt: str
    duration: int = 5
    fps: int = 24
    workflow_id: str = DEFAULT_TEXT_VIDEO_WORKFLOW_ID
    mode: str = "text"
    resolution: str = "540p"
    aspect_ratio: str = "16:9"
    platform_mode: str = "auto"
...
def _video_resolution_height(resolution: str) -> int:
    ...
    return heights.get(res, 540)


def _video_dimensions(resolution: str, aspect_ratio: str) -> tuple[int, int]:
    aspect = str(aspect_ratio or "16:9").strip().lower()
    height = _video_resolution_height(resolution)
    if aspect in {"source", "auto", "image", "same"}:
        aspect = "16:9"
    ratio_match = re.match(r"^\s*(\d+(?:\.\d+)?)\s*[:/x]\s*(\d+(?:\.\d+)?)\s*$", aspect)
    if ratio_match:
        w_ratio = float(ratio_match.group(1))
        h_ratio = float(ratio_match.group(2))
        if w_ratio > 0 and h_ratio > 0:
            return int(round(height * w_ratio / h_ratio)), height
    if aspect == "9:16":
...
def _get_nexus_handlers() -> CinesmithMCPHandlers:
    global _NEXUS_HANDLERS
    if _NEXUS_HANDLERS is None:
        _NEXUS_HANDLERS = CinesmithMCPHandlers(REPO_ROOT)
...
async def api_local_spark_media_styles():
    """Spark media style presets implemented locally through Cinesmith."""
...
async def api_local_spark_media_generate_video(req: LocalSparkMediaVideoRequest):
    ...
    prompt, _ = apply_model_prompt_standard(
        req.prompt,
        workflow_id=DEFAULT_VIDEO_WORKFLOW_ID,
        model_family="ltx",
        render_type="video",
    )
...
async def api_video_process(req: VideoProcessRequest):
    ...
    requested_aspect = str(req.aspect_ratio or "16:9").strip().lower()
    use_source_aspect = requested_aspect in {"source", "auto", "image", "same"}
    if use_source_aspect:
        requested_width = None
        requested_height = _video_resolution_height(req.resolution)
    else:
        requested_width, requested_height = _video_dimensions(req.resolution, requested_aspect)
    service = HermesVideoService(
        media_videos=MEDIA_VIDEOS,
    ...
    result = await service.process(
        ...
        source_aspect=use_source_aspect,
    ...
    result["aspect_ratio"] = "source" if use_source_aspect else requested_aspect
    result["resolution"] = req.resolution
    return result


async def api_video_process_text(req: VideoTextProcessRequest):
    prompt = (req.prompt or "").strip()
    if not prompt:
        raise HTTPException(status_code=400, detail="prompt is required")

    platform_skill = detect_platform_skill(prompt, requested_mode=req.platform_mode)
    if platform_skill.get("active"):
        constraints = platform_skill.get("constraints") or {}
        if int(req.duration or 0) < int(constraints.get("duration_min_sec", 0)):
            req.duration = int(constraints.get("duration_default_sec", req.duration or 5))
        if not req.fps:
            req.fps = int(constraints.get("fps", 24))

    requested_width, requested_height = _video_dimensions(req.resolution, req.aspect_ratio)
    constraints = platform_skill.get("constraints") if isinstance(platform_skill, dict) else {}
    if isinstance(constraints, dict):
        requested_width = int(constraints.get("width") or requested_width)
        requested_height = int(constraints.get("height") or requested_height)

    preflight = _video_workflow_preflight(req.workflow_id or DEFAULT_TEXT_VIDEO_WORKFLOW_ID)
    if not preflight.get("available"):
        raise HTTPException(status_code=404, detail=f"Workflow not found: {preflight.get('workflow_id')}")
    workflow_id = str(preflight["workflow_id"])
    if workflow_id == DEFAULT_VIDEO_WORKFLOW_ID:
        workflow_id = DEFAULT_TEXT_VIDEO_WORKFLOW_ID
        preflight = _video_workflow_preflight(workflow_id)
        if not preflight.get("available"):
            raise HTTPException(status_code=404, detail=f"Workflow not found: {workflow_id}")
    ...
    campaign_id = _safe_campaign_name(_ACTIVE_CAMPAIGN or "text_video_batch") or "text_video_batch"
    shot_id = f"{campaign_id}__TEXT_VIDEO_{uuid.uuid4().hex[:8]}"
    service = HermesVideoService(
        media_videos=MEDIA_VIDEOS,
        active_campaign_getter=lambda: campaign_id,
        find_shot=_find_shot,
    ...
    if result.get("status") == "error":
        err = str(result.get("error") or "video_text_process_error")
    ...
    text_shot = {
        ...
        "video_prompt": prompt,
        "video_prompt_source": "text_to_video",
        "image_path": "",
        "image_url": "",
        "video_status": "queued",
        "video_workflow_id": workflow_id,
        "video_duration": int(req.duration or 0),
        "video_fps": int(req.fps or 0),
        "platform_skill": platform_skill,
        "platform_id": str(platform_skill.get("id") or "") if isinstance(platform_skill, dict) else "",
        "platform_constraints": constraints if isinstance(constraints, dict) else {},
        "created_at": now,
        "updated_at": now,
    }
    for item in result.get("results", []) if isinstance(result.get("results"), list) else []:
        if isinstance(item, dict) and item.get("status") == "ok":
            text_shot["video_prompt_id"] = item.get("prompt_id") or ""
            text_shot["video_seed"] = item.get("seed")
    _SHOTS_STORE.append(text_shot)
    _record_pipeline_event(
        "text_to_video_queued",
        shot_id=shot_id,
        campaign_id=campaign_id,
        workflow_id=workflow_id,
        source="video",
        success=True,
        extra={"duration": req.duration, "fps": req.fps, "resolution": req.resolution, "aspect_ratio": req.aspect_ratio},
    )
    result["workflow_preflight"] = preflight
    ...
    async with httpx.AsyncClient(timeout=15.0) as http:
        for job in req.jobs:
            ...
            try:
                ...
                status = entry.get("status", {}) if isinstance(entry.get("status"), dict) else {}
                compact_error = _compact_comfy_error(status) if status.get("status_str") == "error" else {}
                if status.get("status_str") == "error" and not _has_video_output(entry):
                    shot = _find_shot(shot_id)
                    if shot:
                        shot["video_status"] = "error"
                ...
                if compact_error:
                    shot = _find_shot(shot_id)
                    if shot:
                        shot["video_error"] = compact_error.get("error", "")
                        shot["video_error_detail"] = (
                            "Recovered saved video despite downstream Comfy error: "
                            + compact_error.get("detail", "")
                        ).strip()
                        shot["video_error_node"] = " / ".join([x for x in (compact_error.get("node", ""), compact_error.get("node_type", "")) if x])
                        _persist_media_shot_metadata(shot)
                results.append({
                    "shot_id": shot_id,
                    "prompt_id": prompt_id,
                    ...
                    "campaign_id": campaign_id,
                    "saved_files": video_saved,
                    "video_url": (attached or {}).get("video_url", "") or _media_video_url_for_path(Path(video_saved[0])),
                    "recovered_from_error": bool(compact_error),
                    "warning": compact_error.get("detail", "") if compact_error else "",
                })
            except Exception as e:
...
def _aggregate_audit_results(pass_a: Dict[str, Any], pass_b: Dict[str, Any], prompt: str = "") -> Dict[str, Any]:
    ...
    min_score = _safe_float(os.getenv("CINESMITH_AUDIT_MIN_SCORE", "80"), 80.0)
    min_conf = _safe_float(os.getenv("CINESMITH_AUDIT_MIN_CONFIDENCE", "0.55"), 0.55)
...
async def _run_vision_audit_pass(
    *,
    ...
    payload = {
        ...
        "temperature": 0,
        "max_tokens": int(os.getenv("CINESMITH_VISION_AUDIT_MAX_TOKENS", "16384")),
...
async def audit_render_with_kimi_vl(image_path: str, prompt: str = "", campaign: str = "default"):
    ...
    try:
        ...
        print(
            "[CINESMITH] [KIMI-VL] Audit result: "
        ...
        kimi_error = str(e)
        print(f"[CINESMITH] [KIMI-VL] Audit failed: {kimi_error}")
        return {
            "score": 0,
            "passed": False,
...
async def write_audit_to_memory(audit_result: dict, image_path: str, prompt: str = "", campaign: str = "default"):
    ...
    print(f"[CINESMITH] [MEM] Written audit to memory: {event['event_id']} (score={event['audit_score']})")
...
async def api_hermes_teach(req: TeachModeRequest):
    ...
    """
    if os.getenv("CINESMITH_ENABLE_TEACH_MODE", "false").lower() != "true":
        return _legacy_disabled("/api/hermes/teach", "CINESMITH_ENABLE_TEACH_MODE=true")
    from core.hermes.hermes_agent import HermesAgent
...
async def on_startup():
    ...
    if local.is_available:
        ...
        if success:
            print(f"[CINESMITH] LM Studio auto-detected model: {selected} (available: {models})")
            ...
            print("[CINESMITH] LM Studio reachable but no models loaded")
            await emit_hermes_event("lmstudio_empty", {"host": local.base_url})
    else:
        print(f"[CINESMITH] LM Studio not reachable at startup: {local.base_url}")
...
async def _test_chat_completion(endpoint: str, api_key: str, model: str) -> Dict[str, Any]:
    ...
    t0 = time.time()
    try:
        timeout = httpx.Timeout(float(os.getenv("CINESMITH_PROVIDER_TEST_TIMEOUT_SEC", "75")), connect=8.0)
        async with httpx.AsyncClient(timeout=timeout) as client:
...
async def api_character_role_prompt(req: CharacterRolePromptRequest):
    ...
    system_prompt = (
        "You are Cinesmith Character Casting Director. Return only strict JSON. "
...
def _default_character_workflow_path(workflow_id: str = "") -> Optional[Path]:
    ...
    candidates.extend([
        REPO_ROOT / "workflows" / "04_flux2_multi_reference_character_sheet.json",
        REPO_ROOT / "workflows" / "02_flux2_multi_reference_character_sheet.json",
...
async def api_render_character(req: RenderCharacterRequest):
    ...
    workflow_path = REPO_ROOT / "workflows" / "hermes_z_image_turbo_api.json"
    if not workflow_path.exists():
        workflow_path = REPO_ROOT / "workflows" / "06_z_image_turbo.json"
    if not workflow_path.exists():
        workflow_path = REPO_ROOT / "workflows" / "z_image_turbo_api.json"
    if not workflow_path.exists():
...
CINESMITH_HERMES_LAUNCHER = str(cinesmith_hermes_launcher())
CINESMITH_HERMES_HOME = str(cinesmith_hermes_home())
...
async def api_hermes_profile_chat(req: HermesProfileChatRequest):
    ...
    if not os.path.exists(CINESMITH_HERMES_LAUNCHER):
        raise HTTPException(status_code=503, detail="Hermes engine not found")
    try:
        skills_list = []
        if profile == 'live': skills_list = ['cinesmith-evolution-plan', 'cinematic_consistency_protocol']
        ...
        # Vendored launcher + isolated HERMES_HOME (never PATH hermes → ~/.hermes).
        cmd_args = cinesmith_hermes_cli_argv("-p", profile)
        if skills_list:
    ...
    async def event_generator():
        try:
            ...
            response = await hermes_brain.chat(
                messages,
                max_tokens=max(512, min(int(req.max_tokens or 2048), 16384)),
                temperature=max(0.1, min(float(req.temperature or 0.8), 1.5)),
            )
            chunk_size = 20
...
# --- Domain routers (system / campaigns / script) ---
# Handlers remain in this module; routers only mount them for structure & OpenAPI tags.
try:
    from dashboard.routes import register_domain_routers

    register_domain_routers(app)
except Exception as _domain_reg_err:  # pragma: no cover
    print(f"[CINESMITH] domain router registration failed: {_domain_reg_err}")
