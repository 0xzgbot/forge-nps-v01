...
from fastapi import FastAPI, WebSocket, WebSocketDisconnect, Query, HTTPException, UploadFile, Form, File
...
WORKSPACE_HERMES_LAUNCHER = REPO_ROOT / "hermes_engine" / "hermes"
WORKSPACE_HERMES_HOME = REPO_ROOT / "hermes_home"
WORKSPACE_HERMES_PYTHON = REPO_ROOT / "hermes_engine" / ".venv" / "bin" / "python"
FORGE_HERMES_COMMAND = os.getenv("FORGE_HERMES_COMMAND", "forgehermes")
logger = logging.getLogger("ForgeDashboard")

app = FastAPI()


def _workspace_hermes_env() -> Dict[str, str]:
    """Environment that forces Hermes to use the Forge-local runtime/home."""
    pythonpath = os.environ.get("PYTHONPATH", "")
    repo_path = str(REPO_ROOT)
    return {
        **os.environ,
        "HERMES_HOME": str(WORKSPACE_HERMES_HOME),
        "FORGE_HERMES_ROOT": str(REPO_ROOT),
        "FORGE_HERMES_LAUNCHER": str(WORKSPACE_HERMES_LAUNCHER),
        "HERMES_QUIET": "1",
        "NO_COLOR": "1",
        "PYTHONPATH": repo_path + (os.pathsep + pythonpath if pythonpath else ""),
    }


def _require_workspace_hermes_runtime() -> Dict[str, Any]:
    """Validate that this app is backed by the Hermes agent bundled in this repo."""
    if not WORKSPACE_HERMES_LAUNCHER.exists():
        raise RuntimeError(f"Forge-local Hermes launcher not found: {WORKSPACE_HERMES_LAUNCHER}")
    if not WORKSPACE_HERMES_HOME.exists():
        raise RuntimeError(f"Forge-local Hermes home not found: {WORKSPACE_HERMES_HOME}")

    command_path = shutil.which(FORGE_HERMES_COMMAND)
    command_uses_workspace = False
    if command_path:
        try:
            command_text = Path(command_path).read_text(encoding="utf-8", errors="ignore")
            command_uses_workspace = (
                str(WORKSPACE_HERMES_LAUNCHER) in command_text
                and str(WORKSPACE_HERMES_HOME) in command_text
            )
        except OSError:
            command_uses_workspace = Path(command_path).resolve() == WORKSPACE_HERMES_LAUNCHER.resolve()

    return {
        "repo_root": str(REPO_ROOT),
        "hermes_home": str(WORKSPACE_HERMES_HOME),
        "launcher": str(WORKSPACE_HERMES_LAUNCHER),
        "command": FORGE_HERMES_COMMAND,
        "command_path": command_path,
        "command_uses_workspace": command_uses_workspace,
    }


def _workspace_hermes_cmd(message: str, profile: str, skills_list: Optional[List[str]] = None) -> List[str]:
    runtime = _require_workspace_hermes_runtime()
    if runtime["command_path"] and runtime["command_uses_workspace"]:
        cmd_args = [runtime["command_path"], "-p", profile]
    else:
        python_bin = str(WORKSPACE_HERMES_PYTHON if WORKSPACE_HERMES_PYTHON.exists() else sys.executable)
        cmd_args = [python_bin, str(WORKSPACE_HERMES_LAUNCHER), "-p", profile]
    if skills_list:
        cmd_args.extend(["--skills", ",".join(skills_list)])
    cmd_args.extend(["chat", "-Q", "-q", message])
    return cmd_args
...
# Skills loader — lazy-initialized on first API call
_skills_loader: Optional[SkillLoader] = None

def _get_skills_loader() -> SkillLoader:
    global _skills_loader
    if _skills_loader is None:
        try:
            _skills_loader = SkillLoader()
        except Exception as e:
            logger = __import__("logging", fromlist=["getLogger"]).getLogger("Dashboard")
            logger.warning(f"[DASHBOARD] Failed to init SkillLoader: {e}")
            _skills_loader = SkillLoader.__new__(SkillLoader)
            _skills_loader._skills = []
...
@app.get("/api/skills")
async def get_skills(query: str = Query("", description="Search text to match against skill keywords")):
    """List all loaded skills with name, domain, and keyword count.

    If 'query' is provided, returns skills matched against the query text with confidence scores.
    """
    loader = _get_skills_loader()
    if query:
        matched = loader.match(query, max_skills=10)
        return {
            "mode": "search",
            "query": query,
            "skills": [
                {
                    ...
                    "confidence": len([kw for kw in s["keywords"] if kw in query.lower()]) / max(len(s["keywords"]), 1),
                }
                for s in matched
            ],
            "total_loaded": len(loader._skills),
        }
    else:
        return {
            "mode": "list",
...
async def api_memory_stats():
    """Memory statistics for dashboard cards. Merges memory_api stats with hermes_agent source counts."""
    stats = get_memory_stats()

    # Add source breakdown from HermesAgent (canonical source of truth)
    try:
        from core.hermes.hermes_agent import HermesAgent
        hermes = HermesAgent()
        summary = hermes.get_memory_summary()
        episodic = summary.get("episodic_stats", {})
        for key in ("live_count", "demo_count", "seed_count", "promotable_count"):
            if key not in stats:
                stats[key] = episodic.get(key, 0)
    except Exception as e:
        logger.warning(f"Failed to load hermes_agent stats for /api/memory/stats: {e}")
...
# --- Spark / Render Batch Endpoints ---

_render_jobs: Dict[str, Any] = {}
...
async def _dispatch_single(
    job_id: str, idx: int, workflow_path: str, seed: int, anchor: Optional[str], prompt: str = "",
    campaign: str = "", run_audit: bool = True
) -> Dict[str, Any]:
    """Dispatch a single render to ComfyUI, poll for completion, download PNG, audit, write memory."""
    import urllib.request, json as _json, time, asyncio
    cfg = _load_config()
    host = cfg.get("comfyui", {}).get("primary", "")
    if not host:
        return {"index": idx, "status": "error", "message": "no ComfyUI host configured"}

    campaign_name = campaign or job_id

    try:
        # Load workflow JSON for prompt structure
        wf_path = Path(workflow_path) if os.path.isabs(workflow_path) else None
        if not wf_path or not wf_path.exists():
            base = REPO_ROOT / "workflows"
            name = workflow_path.split("/")[-1] if "/" in workflow_path else workflow_path
            wf_path = base / name
        if not wf_path.exists():
            wf_path = REPO_ROOT / "data" / "workflows" / name

        prompt_data: Dict[str, Any] = {
            "class_type": "SaveImage",
            "inputs": {"filename_prefix": f"{job_id}_{idx}", "seed": seed}
        }
        if wf_path.exists():
        ...
        actual_seed = seed if seed >= 0 else _rand.randint(1, 2**32 - 1)
        prompt_data.setdefault("inputs", {})["seed"] = actual_seed
        if anchor:
            prompt_data.setdefault("inputs", {})["character_anchor"] = anchor

        # Dispatch to ComfyUI
        payload = _json.dumps(prompt_data).encode()
        req = urllib.request.Request(f"{host}/prompt", method="POST")
        resp = urllib.request.urlopen(req, data=payload, timeout=10)
        prompt_id = _json.loads(resp.read())["prompt_id"]

        # Update job status via WebSocket broadcast
        from .forge_dashboard import manager as _mgr
        await _mgr.broadcast({
            "type": "spark_job_update",
            "job_id": job_id,
            "index": idx,
            "status": "rendering",
            "seed": actual_seed,
        ...
        while time.time() - start_t < max_wait:
            try:
                hist_req = urllib.request.Request(f"{host}/history/{prompt_id}")
                hist_resp = urllib.request.urlopen(hist_req, timeout=5)
                history = _json.loads(hist_resp.read())
        ...
        if not img_info:
            ...
            return {"index": idx, "status": "error", "message": "render timed out"}

        filename = img_info.get("filename", "")
        subfolder = img_info.get("subfolder", "")

        # --- DOWNLOAD PNG from ComfyUI ---
        local_dir = REPO_ROOT / "data" / "renders" / "campaigns" / campaign_name
        local_dir.mkdir(parents=True, exist_ok=True)
        local_path = local_dir / filename

        view_url = f"{host}/view?filename={filename}&type=output"
        if subfolder:
            view_url += f"&subfolder={subfolder}"
        view_req = urllib.request.Request(view_url)
        view_resp = urllib.request.urlopen(view_req, timeout=10)
        with open(local_path, "wb") as f:
            f.write(view_resp.read())

        rel_path = f"data/renders/campaigns/{campaign_name}/{filename}"
        image_src = f"/renders/campaigns/{campaign_name}/{filename}"

        # --- STREAM render_complete event with local image src ---
        await _mgr.broadcast({
        ...
        print(f"[FORGE] Downloaded render: {local_path}")
        ...
        if run_audit:
            try:
                ...
                bridge = KimiBridge()
                # Find character anchor image (elara_vance.jpg or similar)
                anchor_dir = REPO_ROOT / "data" / "character_banks" / "anchors"
                anchor_path = None
                if anchor:
                    # Try to find anchor image for this character
                    for ext in [".jpg", ".png", ".jpeg"]:
                        candidate = anchor_dir / f"{anchor}{ext}"
                        if candidate.exists():
                            anchor_path = str(candidate)
                            break
                # Fallback: find any anchor image
                if not anchor_path and anchor_dir.exists():
                    for f in sorted(anchor_dir.iterdir()):
                        if f.suffix.lower() in [".jpg", ".png", ".jpeg"]:
                            anchor_path = str(f)
                            break

                if anchor_path:
                    audit_result = await bridge.audit_image(
                        image_path=str(local_path),
                        reference_paths=[anchor_path],
                        shot_description=prompt or f"Shot {idx} from campaign {campaign_name}",
                    )
                else:
                    # Fallback: generic quality audit
                    audit_result = await audit_render_with_kimi_vl(
                        str(local_path), prompt, campaign_name
                    )

                # --- STREAM [KIMI-VL] event ---
                await emit_hermes_event("kimi_vl_audit", {
                    ...
                    "image_path": str(local_path),
                    "campaign": campaign_name,
                    "score": audit_result.get("overall_score", audit_result.get("score", 0)),
                    "passed": audit_result.get("passed", audit_result.get("is_consistent", False)),
                    "confidence": audit_result.get("confidence", 0),
                    "feedback": audit_result.get("feedback", audit_result.get("mismatch_details", "")),
                    "issues": audit_result.get("issues", audit_result.get("errors", [])),
                })
                print(f"[FORGE] [KIMI-VL] Audit: score={audit_result.get('overall_score', audit_result.get('score', 'N/A'))}, passed={audit_result.get('passed', audit_result.get('is_consistent', 'N/A'))}")

                # --- WRITE AUDIT TO MEMORY (source=live) ---
                await write_audit_to_memory(
                    audit_result, str(local_path),
                    prompt or f"Shot {idx} from campaign {campaign_name}",
                    campaign_name
                )
            except Exception as audit_err:
                print(f"[FORGE] [KIMI-VL] Audit failed: {audit_err}")
                await _mgr.broadcast({
                    "type": "spark_job_update",
                    "job_id": job_id,
                    "index": idx,
                    "status": "audit_failed",
                    "message": str(audit_err),
                })

        return {
            "index": idx, "status": "complete",
            "image_url": image_src, "image_path": str(local_path),
            "filename": filename, "seed": actual_seed,
            "campaign": campaign_name,
            "audit": audit_result,
        }

    except Exception as e:
        from .forge_dashboard import manager as _mgr2
        await _mgr2.broadcast({
            "type": "spark_job_update",
            "job_id": job_id,
            "index": idx,
            "status": "error",
...
async def api_workflows():
    """Scan and list workflow JSON files."""
    import glob as _glob
    candidates = [REPO_ROOT / "workflows", REPO_ROOT / "data" / "workflows"]
    results = []
    for d in candidates:
        if d.exists():
            results.extend(sorted(str(p) for p in d.glob("*.json")))
    return sorted(set(results))


@app.get("/api/spark/output-path")
async def api_spark_output_path():
    """Get the output directory path for Spark renders."""
    campaign_dir = REPO_ROOT / "data" / "campaigns"
    return {"path": str(campaign_dir)}


@app.post("/api/render/batch")
async def api_render_batch(payload: Dict[str, Any]):
    """Dispatch a batch of renders to ComfyUI — supports concurrent dispatch with audit."""
    import random as _rand, uuid as _uuid

    workflow = payload.get("workflow", "")
    count = max(1, int(payload.get("count", 6)))
    seeds = payload.get("seeds") or [_rand.randint(-1, 2**32 - 1) for _ in range(count)]
    if len(seeds) < count:
        seeds = list(seeds) + [-1] * (count - len(seeds))
    anchor = payload.get("anchor_character")
    prompt = payload.get("prompt", "")
    campaign = payload.get("campaign", "")
    run_audit = payload.get("audit", True)  # Enable Kimi-VL audit by default

    job_id = f"batch_{_uuid.uuid4().hex[:8]}"
    campaign_name = campaign or job_id

    campaign_dir = REPO_ROOT / "data" / "campaigns" / campaign_name
    ...
    # Dispatch all renders concurrently using asyncio.gather
    tasks = [
        _dispatch_single(
            job_id, i, workflow, seeds[i] if i < len(seeds) else -1,
            anchor, prompt, campaign=campaign_name, run_audit=run_audit
        )
        for i in range(count)
    ]
    results = await asyncio.gather(*tasks, return_exceptions=True)

    completed = sum(1 for r in results if isinstance(r, dict) and r.get("status") == "complete")
    errored = len(results) - completed

    _render_jobs[job_id]["status"] = "completed" if errored == 0 else "partial"
    _render_jobs[job_id]["results"] = [r if isinstance(r, dict) else {"index": i, "status": "error", "message": str(r)} for i, r in enumerate(results)]

    from .forge_dashboard import manager as _mgr3
    await _mgr3.broadcast({
        "type": "spark_job_update",
        "job_id": job_id,
        "status": "done",
        "completed": completed,
...
@app.post("/api/render/batch/clear")
async def api_render_batch_clear():
    """Clear all in-memory batch jobs."""
    _render_jobs.clear()
    return {"status": "cleared"}


@app.get("/api/spark/jobs")
async def api_spark_jobs(job_id: Optional[str] = None):
    """Get render job status."""
    if job_id:
        return _render_jobs.get(job_id, {"status": "not_found"})
    return dict(_render_jobs)


@app.post("/api/comfyui/clear")
async def api_comfyui_clear():
    """Clear ComfyUI queue via the configured primary host."""
    import urllib.request as _ur
    cfg = _load_config()
    host = cfg.get("comfyui", {}).get("primary", "")
    if not host:
        return {"status": "error", "message": "no ComfyUI host configured"}
    try:
        req = _ur.Request(f"{host}/queue", method="POST")
        data = json.dumps({"clear": True}).encode()
        _ur.urlopen(req, data=data, timeout=5)
        return {"status": "cleared"}
    except Exception as e:
...
_data_renders_dir.mkdir(parents=True, exist_ok=True)
...
async def api_script_reparse(req: ReparseRequest = None):
    ...
    if req and req.path:
        ...
        # Read active brief first (working copy), fall back to creative_brief.md, then world_bible.md
        for candidate in ("active_brief.md", "creative_brief.md", "world_bible.md"):
            script_path = repo_root / "data" / "lore_bible" / candidate
...
async def api_parse_with_kimi(req: ParseScriptRequest = None):
    ...
    if req and req.path:
        ...
        for candidate in ("active_brief.md", "creative_brief.md", "world_bible.md"):
            ...
            raise HTTPException(status_code=404, detail="No brief found in data/lore_bible/")
...
# Character anchor manager — loaded once at module level
_character_anchor_mgr = None

def _get_character_anchor_mgr():
    global _character_anchor_mgr
    if _character_anchor_mgr is None:
        from core.character.anchor_manager import CharacterAnchorManager
        _character_anchor_mgr = CharacterAnchorManager()
    return _character_anchor_mgr


def _refresh_anchor_manager():
    """Re-scan the anchors directory and rebuild the character lookup."""
    global _character_anchor_mgr
    from core.character.anchor_manager import CharacterAnchorManager
    _character_anchor_mgr = CharacterAnchorManager()
    print(f"[FORGE] [ANCHORS] Refreshed anchor manager: {len(_character_anchor_mgr.list_known_characters())} anchors indexed")
...
async def audit_render_with_kimi_vl(image_path: str, prompt: str = "", campaign: str = "default"):
    ...
    Character anchor flow:
    1. Extract character names from prompt
    2. Load matching anchor images from character banks
    3. If anchors found: use KimiBridge.audit_image() with anchors as references
    4. If no anchors: fall back to generic quality audit via KimiVLClient
    5. Log warnings for characters without anchors
    """
    ...
    anchor_mgr = _get_character_anchor_mgr()
    anchor_info = anchor_mgr.get_audit_references(prompt)

    # Log anchor warnings
    for warning in anchor_info.get("warnings", []):
        print(f"[FORGE] [ANCHORS] {warning}")

    reference_paths = anchor_info.get("reference_paths", [])
    characters = anchor_info.get("characters", [])

    if reference_paths and characters:
        # Use KimiBridge.audit_image() with character anchors as references
        print(
            f"[FORGE] [ANCHORS] Auditing render with {len(reference_paths)} character anchor(s): "
            f"{', '.join(characters)}"
        )
        ...
        try:
            ...
            # Map KimiBridge.audit_image() result to dashboard schema
            return {
                "score": audit_result.get("overall_score", audit_result.get("score", 50)),
                "passed": audit_result.get("passed", audit_result.get("is_consistent", False)),
                "feedback": audit_result.get("feedback", audit_result.get("mismatch_details", "")),
                "issues": audit_result.get("issues", audit_result.get("errors", [])),
                "characters_checked": characters,
                "anchors_used": [str(p) for p in reference_paths],
                "anchor_warnings": anchor_info.get("warnings", []),
            }
        except Exception as e:
            print(f"[FORGE] [ANCHORS] Anchor audit failed, falling back to generic audit: {e}")

    # Fallback: generic quality audit via KimiVLClient (no character anchors)
    if characters and not reference_paths:
        print(
            f"[FORGE] [ANCHORS] Characters detected ({', '.join(characters)}) but no anchors found. "
            f"Running generic quality audit only."
        )
    ...
    try:
        ...
        result["characters_checked"] = characters
        result["anchors_used"] = []
        result["anchor_warnings"] = anchor_info.get("warnings", [])
        return result
    except Exception as e:
...
async def write_audit_to_memory(audit_result: dict, image_path: str, prompt: str = "", campaign: str = "default"):
    """
    Write audit result to episodic memory using EpisodicMemory.record() and emit [MEM] event.
    Uses the validated write boundary: source=live, event_type=audit_result.
    """
    from core.hermes.memory.episodic_memory import EpisodicMemory
    ...
    mem = EpisodicMemory(memory_dir=str(episodic_dir))
    ...
    event_payload = {
        ...
        "success": audit_result.get("passed", audit_result.get("is_consistent", False)),
        "audit_score": audit_result.get("overall_score", audit_result.get("score", 0)),
        "error_category": "quality_fail" if not audit_result.get("passed", audit_result.get("is_consistent", False)) else "none",
        "fix_applied": "",
    ...
    event_id = mem.record(event_payload, source="live")

    print(f"[FORGE] [MEM] Written audit to memory: {event_id} (score={event_payload['audit_score']})")

    # Emit [MEM] event
    await emit_hermes_event("memory_written", {
        "concept": f"Audit: {prompt[:50] if prompt else 'render'}",
        "event_id": event_id,
        "score": event_payload["audit_score"],
        "passed": event_payload["success"],
        "feedback": event_payload["feedback"][:100],
    })

    return {"event_id": event_id, **event_payload}
...
async def api_hermes_teach(req: TeachModeRequest):
    ...
    episodic.record(failure_event, source="demo")
    ...
    episodic.record(success_event, source="demo")
...
async def on_startup():
    runtime = _require_workspace_hermes_runtime()
    os.environ.update(_workspace_hermes_env())
    print(
        "[FORGE] Workspace Hermes runtime: "
        f"home={runtime['hermes_home']} launcher={runtime['launcher']} "
        f"command={runtime['command_path'] or 'direct launcher'}"
    )
...
def _normalize_char(char: Dict[str, Any]) -> Dict[str, Any]:
    """Flatten DNA fields to top-level for consistent API response."""
    dna = char.get("dna") or {}
    return {
        "id": char.get("id", ""),
        "name": char.get("name", ""),
        "description": char.get("description", dna.get("bio", "")),
        "role": char.get("role", ""),
        "eyes": dna.get("eyes", ""),
        "hair": dna.get("hair", ""),
        "build": dna.get("build", ""),
        "clothing": dna.get("clothing", ""),
        "signature_item": dna.get("signature", ""),
        "palette": dna.get("palette", []),
        "anchor_image": char.get("anchor_url", ""),
        "anchor_prompt": char.get("anchor_prompt", ""),
        "score": char.get("score", 0),
        "accent": char.get("accent", "cyan"),
    }
...
async def api_get_characters():
    ...
    return [_normalize_char(c) for c in _CHARACTERS_STORE.values()]


@app.get("/api/characters/anchors")
async def api_get_character_anchors():
    """
    List all character anchors from the anchor manager.

    Returns:
    - known_characters: characters with anchors in data/character_banks/anchors/
    - store_characters: characters in the store, with has_anchor flag
    - missing_anchors: characters in the store that don't have anchors yet
    """
    mgr = _get_character_anchor_mgr()
    known = mgr.list_known_characters()

    # Cross-reference with character store
    store_chars = []
    missing = []
    for char_id, char in _CHARACTERS_STORE.items():
        has_anchor = mgr.has_anchor(char.get("name", char_id))
        entry = {
            "id": char_id,
            "name": char.get("name", char_id),
            "has_anchor": has_anchor,
            "anchor_url": char.get("anchor_url", ""),
            "description": char.get("description", "")[:100],
        }
        store_chars.append(entry)
        if not has_anchor:
            missing.append(entry)

    return {
        "known_anchors": known,
        "store_characters": store_chars,
        "missing_anchors": missing,
        "total_known": len(known),
        "total_store": len(store_chars),
...
async def api_get_character_variations(char_id: str):
    ...
    # Scan campaigns folder recursively (data/campaigns/**)
    campaigns_root = repo_root / "data" / "campaigns"
    if campaigns_root.exists():
        char_slug = char_id.lower()
        for campaign_dir in sorted(campaigns_root.iterdir()):
            ...
            for f in sorted(campaign_dir.rglob("*")):
                if f.is_file() and f.suffix.lower() in (".png", ".jpg", ".jpeg", ".webp"):
                    meta_path = f.with_suffix(f.suffix + ".json")
                    meta = {"prompt": "", "score": 0, "type": "campaign"}
                    if meta_path.exists():
                        try:
                            with open(meta_path) as mf:
                                meta = json.load(mf)
                            ...
                            pass
                    # Match by filename stem or metadata
                    file_stem = f.stem.lower()
                    prompt_text = str(meta.get("prompt", "")).lower()
                    char_name_slug = char_id.lower().replace("_", " ")
                    if (char_id.lower() in file_stem or
                        char_slug in file_stem or
                        char_name_slug in file_stem or
                        char_slug in prompt_text):
                        rel = f.relative_to(campaigns_root)
                        variations.append({
                            "id": f.stem,
                            "src": "/campaigns/" + str(rel),
                            "type": meta.get("type", "campaign"),
                            "score": meta.get("score", 0),
                            "seed": meta.get("seed", 0),
                            "prompt": meta.get("prompt", ""),
                            "campaign": campaign_dir.name,
    ...
    # Scan renders/campaigns folder recursively (data/renders/campaigns/**)
    renders_campaign_root = repo_root / "data" / "renders" / "campaigns"
    if renders_campaign_root.exists():
        char_slug = char_id.lower()
        for campaign_dir in sorted(renders_campaign_root.iterdir()):
...
class GenerateAnchorRequest(BaseModel):
    name: str = ""
    description: str = ""


@app.post("/api/characters/{char_id}/generate-anchor")
async def api_generate_anchor(char_id: str, req: GenerateAnchorRequest = None):
    """Generate an anchor image for a character using ComfyUI render pipeline."""
    from core.dispatch.comfy_client import ComfyUIClient
    from core.bridge.nous_hermes_bridge import NousHermesBridge
    ...
    if req:
        if req.description:
            prompt = f"Portrait of {char_id}, {req.description}"
        elif req.name:
            prompt = f"Portrait of {req.name}, photorealistic, high detail, 8k resolution"
    if not prompt and char:
        prompt = char.get("anchor_prompt", "") or f"Portrait of {char.get('name', char_id)}, photorealistic portrait, high detail, 8k resolution"
    if not prompt:
        raise HTTPException(status_code=400, detail="No description available for anchor generation")

    # Use the same render pipeline as character rendering
    workflow_path = Path("/Users/zgbot/workflows/spark_image_z_image_turbo.json")
    if not workflow_path.exists():
    ...
    safe_name = re.sub(r'[^a-z0-9]+', '_', char_id.lower()).strip('_')
    ...
    if not filename:
        # Return queued status so UI can poll later
        _CHARACTERS_STORE[char_id]["anchor_url"] = ""  # mark as pending
    ...
    if char_id in _CHARACTERS_STORE:
        _CHARACTERS_STORE[char_id]["anchor_url"] = anchor_url
        _persist_character(char_id, _CHARACTERS_STORE[char_id])

    # Refresh the CharacterAnchorManager index so audits pick up the new anchor
    _refresh_anchor_manager()

    return {"status": "complete", "anchor_url": anchor_url, "prompt_id": prompt_id}


class CreateAnchorRequest(BaseModel):
    """Create a new character with an anchor image."""
    name: str = ""
    description: str = ""


@app.post("/api/characters/generate-anchor")
async def api_create_anchor(req: CreateAnchorRequest):
    """
    Create a new character and generate an anchor image for them.

    Flow:
    1. Generate character DNA via NousHermesBridge (if description provided)
    2. Build anchor prompt from character DNA
    3. Submit to ComfyUI, poll for completion
    4. Save anchor to data/character_banks/anchors/{name}.jpg
    5. Register character in store + refresh anchor manager
    """
    from core.dispatch.comfy_client import ComfyUIClient
    from core.bridge.nous_hermes_bridge import NousHermesBridge
    ...
    anchors_dir = Path(__file__).parent.parent / "data" / "character_banks" / "anchors"
    existing = anchors_dir / f"{safe_name}.jpg"
    if existing.exists():
        return {
            "status": "already_exists",
            "anchor_url": f"/api/characters/anchor/{safe_name}",
            "message": f"Anchor for '{name}' already exists at {existing}"
        }

    # Generate anchor prompt — use Hermes for richer prompts
    description = req.description.strip()
    if description:
        # Use NousHermesBridge to generate a character profile with anchor prompt
        try:
            bridge = NousHermesBridge()
            char_data = await bridge.generate_character(description)
            if char_data and char_data.get("anchor_prompt"):
                prompt = char_data["anchor_prompt"]
            else:
                prompt = (
                    f"Character anchor portrait of {name}, {description}. "
                    f"Front-facing, neutral expression, studio lighting, "
                    f"high detail, 8k resolution, photorealistic."
                )
        except Exception:
            prompt = (
                f"Character anchor portrait of {name}, {description}. "
        ...
        prompt = (
            f"Character anchor portrait of {name}. "
            f"Front-facing, neutral expression, studio lighting, "
            f"high detail, 8k resolution, photorealistic."
        )
    ...
    filename = await client.poll_job(prompt_id, timeout_sec=300)
    if not filename:
        return {"status": "queued", "prompt_id": prompt_id, "character_name": name}

    # Download and save anchor
    anchors_dir.mkdir(parents=True, exist_ok=True)
    saved = await client.download_outputs(prompt_id, str(anchors_dir))
    if not saved:
        raise HTTPException(status_code=500, detail="Image download from Spark failed")
    ...
    _CHARACTERS_STORE[safe_name] = char_entry
    _persist_character(safe_name, char_entry)

    # Refresh anchor manager index
    _refresh_anchor_manager()
...
async def api_create_character(
    ...
    creative_brief_path = Path(__file__).parent.parent / "data" / "lore_bible" / "creative_brief.md"
    try:
        if creative_brief_path.exists():
            wb_text = creative_brief_path.read_text(encoding="utf-8")
        else:
            wb_text = ""
        ...
        if not creative_brief_path.exists():
            ...
            wb_text = "# Creative Brief: CHARACTER ROSTER\n\n" + char_section
        ...
        creative_brief_path.write_text(wb_text, encoding="utf-8")

        # Also sync to active_brief.md (the working brief for pipeline runs)
        active_path = creative_brief_path.parent / "active_brief.md"
        try:
            if active_path.exists():
                active_text = active_path.read_text(encoding="utf-8")
            else:
                # Copy from creative_brief.md as base
                active_text = wb_text
            active_char_section = (
                f"\n## KEY CHARACTER: {name.strip().upper()}\n"
                f"- **Role:** {description or 'Character'}\n"
            ...
            if "## KEY CHARACTER:" not in active_text:
                active_text += "\n" + active_char_section
            else:
                conflict_marker = "## CORE CONFLICT"
                if conflict_marker in active_text:
                    active_text = active_text.replace(conflict_marker, active_char_section + conflict_marker)
                else:
                    active_text += "\n" + active_char_section
...
async def api_upload_creative_brief(file: UploadFile = File(...)):
    """Upload a Creative Brief file (.md, .txt, or .pdf)."""
    valid_exts = {'.md', '.txt', '.pdf'}
    ext = Path(file.filename or '').suffix.lower()
    if ext not in valid_exts:
        raise HTTPException(status_code=400, detail=f"Accepted formats: {', '.join(sorted(valid_exts))}")

    cb_dir = Path(__file__).parent.parent / "data" / "lore_bible"
    cb_dir.mkdir(parents=True, exist_ok=True)
    dest = cb_dir / "creative_brief" + ext
    content = await file.read()

    # PDFs are stored as-is; text files are validated as UTF-8
    if ext in ('.md', '.txt'):
        try:
            decoded = content.decode('utf-8')
            dest.write_text(decoded, encoding='utf-8')
            # Also sync to active_brief.md (the working brief for pipeline)
            active_dest = cb_dir / "active_brief.md"
            active_dest.write_text(decoded, encoding='utf-8')
    ...
    return {"status": "saved", "filename": dest.name, "size": len(content)}


@app.get("/api/creative-brief")
async def api_get_creative_brief():
    """Return the current Creative Brief content."""
    cb_dir = Path(__file__).parent.parent / "data" / "lore_bible"
    for ext in ('.md', '.txt'):
        path = cb_dir / f"creative_brief{ext}"
        if path.exists():
            return {"content": path.read_text(encoding='utf-8'), "filename": path.name}
    # Fallback to legacy world bible
    for ext in ('.md',):
        path = cb_dir / f"world_bible{ext}"
        if path.exists():
            return {"content": path.read_text(encoding='utf-8'), "filename": path.name, "legacy": True}
    raise HTTPException(status_code=404, detail="No Creative Brief found")
...
async def api_hermes_profile_chat(req: HermesProfileChatRequest):
    ...
    try:
        ...
        proc = await asyncio.create_subprocess_exec(
            *_workspace_hermes_cmd(req.message, profile, skills_list),
            stdout=asyncio.subprocess.PIPE,
            stderr=asyncio.subprocess.PIPE,
            cwd=str(REPO_ROOT),
            env=_workspace_hermes_env(),
        )
        try:
...
@app.get("/api/hermes/runtime")
async def api_hermes_runtime():
    """Expose the Hermes runtime the Forge app is pinned to."""
    try:
        return {"status": "ready", **_require_workspace_hermes_runtime()}
...
async def api_hermes_chat_stream(req: HermesChatRequest):
    ...
    async def event_generator():
        try:
            ...
            # Fallback: use the Forge-local Hermes CLI launcher.
            skills_list = []
            if profile == 'live': skills_list = ['forge-nps-evolution-plan', 'cinematic_consistency_protocol']
            elif profile == 'character': skills_list = ['character-dna-standardization']
            elif profile == 'script': skills_list = ['narrative-beat-synthesis']
