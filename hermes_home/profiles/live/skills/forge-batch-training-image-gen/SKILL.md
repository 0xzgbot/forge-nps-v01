...
### 1. Prerequisites
- A valid `PROJECT.md` file exists in `~/Desktop/forge/projects/[slug]/`.
- All required character bank files exist in `~/Desktop/forge/projects/[slug]/banks/` (`pose_bank.txt`, `view_bank.txt`, etc.).
- `z_image_turbo_api.json` is available in `~/Desktop/forge/workflows/`.
