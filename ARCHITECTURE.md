...
- **`~/Desktop/forge_nps_v01/`** — original forge project with full orchestrator, agents, memory system, and demo.py
- **`~/dashboard/`** + **`~/core/`** — active Command Center dashboard (port 7000), imports from `~/core/`
...
[KIMI K2-INSTRUCT] moonshotai/kimi-k2-instruct via NVIDIA NIM
    ...
    Stream: {type: "kimi", text: "Shot list ready: 6 shots"}
        │
        ▼ (per shot, sequential)
[HERMES/QWEN] qwen3.6-35b-a3b via LM Studio (localhost:1234)
    ...
    Fallback: if LM Studio offline → use Kimi description + style suffix
        │
        ▼
[COMFYUI SPARK] localhost:8188
    Loads:  ~/workflows/hermes_z_image_turbo_api.json
...
| LM Studio | `localhost:1234` | Hermes/qwen local inference |
| ComfyUI Spark | `localhost:8188` | GPU render cluster |
