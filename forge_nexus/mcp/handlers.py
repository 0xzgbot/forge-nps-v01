...
try:
    from forge_nexus.persistence.engine import SearchEngine
    from forge_nexus.persistence.manager import PersistenceManager
    from forge_nexus.graph.engine import GraphEngine
except ImportError:
    from persistence.engine import SearchEngine
    from persistence.manager import PersistenceManager
    from graph.engine import GraphEngine

class ForgeMCPHandlers:
    """
    The implementation layer that connects MCP tool calls to 
    the actual logic in the Forge Nexus core.
    """
    ...
    def handle_forge_query(self, arguments: Dict[str, Any]) -> Dict[str, Any]:
        """Tool: forge_query - Semantic keyword search."""
    ...
    def handle_forge_context(self, arguments: Dict[str, Any]) -> Dict[str, Any]:
        """Tool: forge_context - Retrieve full metadata for an asset."""
        ...
        import sqlite3
        with sqlite3.connect(self.project_path / ".forge-nexus" / "forge.db") as conn:
    ...
    def handle_forge_impact(self, arguments: Dict[str, Any]) -> Dict[str, Any]:
        """Tool: forge_impact - Find all entities affected by a change."""
        asset_id = arguments.get("asset_id")
        if not asset_id:
            return {"error": "Missing required argument: 'asset_id'"}
    ...
    def handle_forge_trace(self, arguments: Dict[str, Any]) -> Dict[str, Any]:
        """Tool: forge_trace - Discover the connection chain between assets."""
