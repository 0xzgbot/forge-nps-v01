...
class TestCinematicEngine(unittest.TestCase):
    def setUp(self):
        ...
        self.lighting_bank = str(Path(__file__).resolve().parents[1] / "data/character_banks/lighting_bank.txt")
        self.engine = CinematicEngine(self.lighting_bank)
        self.loader = LibraryLoader(str(Path(__file__).resolve().parents[1] / "data/prompt_libraries/"))
