...
You interact with a local ComfyUI server at `http://100.74.164.1:8188 (primary) and http://100.74.164.1:8189 (secondary)` to generate images and video headlessly via the REST API.
...
| `/system_stats` | GET | System info (GPU, VRAM, versions) |
| `/api/history` | GET | Get all job history from server |
| `/queue` | GET | Check queue status |

## Automated Image Collection Workflow

After generating images, automatically collect and organize them into a dedicated folder.

### Auto-Download Script (Add to workflow)

This Python script queries both ComfyUI servers and downloads all generated images:
...
# Configuration
SERVERS = [
    "http://100.74.164.1:8188",  # Primary
    "http://100.74.164.1:8189"   # Secondary
]
OUTPUT_DIR = os.path.expanduser("~/Desktop/comfy_generations")
os.makedirs(OUTPUT_DIR, exist_ok=True)

def download_all_images():
    """Query all servers and download generated images."""
    downloaded_count = 0
...
📥 Querying {server}...")
        try:
            # Get job history
            response = requests.get(f"{server}/api/history", timeout=10)
            if response.status_code == 200:
                ...
                for job in history:
                    ...
                    for output in outputs:
                        ...
                        if filename and "output" in str(filename):
                            # Download the image
                            ...
                            try:
                                ...
                                job_id = str(job.get("prompt_id", "unknown"))[:8]
                                node_type = output.get("node_type", "image")[0:15]
                                
                                safe_filename = f"{timestamp}_{job_id}_{node_type}.png"
...
Add this as a post-generation step:
1. After submitting your workflow, run the script above
2. All images will be automatically downloaded and organized
3. Files are named with: `timestamp_jobid_nodetype.png` for easy sorting
| `/system_stats` | GET | System info (GPU, VRAM, versions) |
| `/api/history` | GET | Get all job history from server |
| `/queue` | GET | Check queue status |

## Server Discovery & Model Finding

When connecting to new servers, use these discovery commands:

```bash
# Test connectivity and get system info
curl -s http://100.74.164.1:8188/system_stats

# List available checkpoint models
curl -s http://100.74.164.1:8188/api/models/checkpoints | python -c "import json,sys; data=json.load(sys.stdin); print('
'.join(data))"

# List diffusion models (for Flux, Wan, etc.)
curl -s http://100.74.164.1:8188/api/models/diffusion | python -c "import json,sys; data=json.load(sys.stdin); print('
'.join(data))"
```

**Common model names found:** `flux2_dev_fp8mixed`, `juggernautXL_ragnarokBy`, `ltx-2.3-22b-dev-fp8`## Discovering Available Models & Loras

Before building a workflow, query the API to find exact model/lora filenames:

```python
# List all available loras
curl -s http://100.74.164.1:8188 (primary) and http://100.74.164.1:8189 (secondary)/object_info/LoraLoader | python -c "
import json,sys; data=json.load(sys.stdin)
for l in data['LoraLoader']['input']['required']['lora_name'][0]: print(l)
"

# List available checkpoints
curl -s http://100.74.164.1:8188 (primary) and http://100.74.164.1:8189 (secondary)/object_info/CheckpointLoaderSimple | python -c "
import json,sys; data=json.load(sys.stdin)
...
curl -s http://100.74.164.1:8188 (primary) and http://100.74.164.1:8189 (secondary)/object_info/UNETLoader | python -c "
import json,sys; data=json.load(sys.stdin)
...
req = urllib.request.Request("http://100.74.164.1:8188 (primary) and http://100.74.164.1:8189 (secondary)/prompt", data=payload, headers={"Content-Type": "application/json"})
...
img_data = urllib.request.urlopen("http://100.74.164.1:8188 (primary) and http://100.74.164.1:8189 (secondary)/view?filename=NAME&type=output").read()
